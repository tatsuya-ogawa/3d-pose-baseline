...
def output_pb():
  ...
  with tf.Session(config=tf.ConfigProto( device_count = device_count )) as sess:
    ...
    #tf.summary.scalar('one_plus_one_summary',model.outputs)
    from tensorflow.python.framework import graph_util
    graph_def = graph_util.convert_variables_to_constants(sess,sess.graph.as_graph_def(),['linear_model/add_1']) 
    tf.train.write_graph(graph_def,'./pb','graph.pb',as_text=False)
    tf.train.write_graph(graph_def,'./pb','graph.txt',as_text=True)
